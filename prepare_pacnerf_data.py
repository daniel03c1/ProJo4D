...
def _prepare_mask(data_folder):
    ...
    matting_model.load_state_dict(
        torch.load("data/checkpoint/pytorch_resnet101.pth", map_location=device)
    )
    matting_model = matting_model.eval().to(torch.float32).to(device)
    with open(data_folder / "all_data.json") as f:
        data_info = json.load(f)
    num_views = len(
        list(set([int(entry["file_path"].split("_")[1]) for entry in data_info]))
    )
    ...
    for entry in data_info:
        stem = posixpath.splitext(posixpath.basename(entry["file_path"]))[0]
        cam_id, frame_id = [int(i) for i in stem.removeprefix("r_").split("_")]
        if frame_id == -1:
            backgroud_all[cam_id] = np.array(
                Image.open(data_folder / entry["file_path"])
            )[..., :3]
    for entry in tqdm(data_info):
        ...
        with torch.no_grad():
            ...
            bgr_tensor = torch.from_numpy(
                bgr.transpose(2, 0, 1).astype(np.float32) / 255
            ).to(device)[None]
...
def prepare_mask(folder):
    try:
        _prepare_mask(folder)
...
if __name__ == "__main__":
    ...
    for folder in glob.glob(os.path.join(args.data_folder, "*")):
        if os.path.exists(os.path.join(folder, "all_data.json")):
            prepare_mask(folder)
        else:
            for subfolder in glob.glob(os.path.join(folder, "*")):
                prepare_mask(subfolder)
