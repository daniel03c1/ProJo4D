...
def readSpringGausCaptureRealInfo(path, white_background, eval, extension=".png"):
    ...
    # Compute optical axis directions in world coordinates
    dirs = []
    ...
    # Determine cube side length from bounding box of original camera centers
    bbox_min = centers.min(axis=0)
    bbox_max = centers.max(axis=0)
    bbox_dims = bbox_max - bbox_min
    max_dim = bbox_dims.max()
    side_len = max_dim * 0.3
    # Generate random points within the cube centered at the closest point
...
def readSpringGausCaptureRealCameras(root_dir, obj_name, white_background):
    ...
    if obj_name in ["burger", "dog"]:  # no checkboard
        postfix = "_plain"
    else:  # ['bun', 'pig', 'potato']
        postfix = "_checker"
    ...
    for name in cam_name:
        # background = Image.open(os.path.join(root_dir, 'dynamic',
        #                                      'backgrounds',
        #                                      f'{name}{postfix}.jpg'))
...
def readGSOInfo(path, config_path, white_background):
    with open(os.path.join(config_path), "r") as f:
        cfg = json.load(f)

    print("Reading GSO data")
    cam_infos = readGSOData(path, white_background, is_eval=False)
    eval_cam_infos = readGSOData(path, white_background, is_eval=True)
...
def readGSOData(
    path, white_background, n_train_frames=16, total_frames=24, dt=0.05, is_eval=False
):
    cam_infos = []
    ...
    for i, cam in enumerate(tqdm(cameras)):
        ...
        for f in range(total_frames):
            if f < n_train_frames:
                if (cam_id < 12) == is_eval:
                    continue
            else:  # f >= n_train_frames
                if not is_eval or (cam_id >= 12):
                    continue

            file_path = f"a_{cam_id}_{f}.png"
            image_path = os.path.join(path, "data", file_path)
...
def readCamerasFromAllData(path, white_background):
    ...
    with open(os.path.join(path, "all_data.json")) as json_file:
        ...
        for idx, frame in enumerate(tqdm(frames)):
            stem = posixpath.splitext(posixpath.basename(frame["file_path"]))[0]
            cam_id, frame_id = stem.removeprefix("r_").split("_")
            if frame_id == "-1":
                continue
...
sceneLoadTypeCallbacks = {
    # colmap dataset reader from official 3D Gaussian
    # [https://repo-sam.inria.fr/fungraph/3d-gaussian-splatting/]
    "Colmap": readColmapSceneInfo,
    # D-NeRF dataset
    # [https://drive.google.com/file/d/1uHVyApwqugXTFuIRRlE4abTW8_rrVeIK/view?usp=sharing]
