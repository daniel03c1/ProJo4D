...
def gen_xyz_list(simulator, frames, diff=True, save_ply=False, path=None):
    ...
    dt = simulator.sim.dt_ori[None]

    while True:
        # assert dt > 5e-5

        simulator.initialize()
        simulator.sim.set_dt(dt)
...
def read_estimation_result(dataset: ModelParams, phys_args, pred_file=None):
    if pred_file is None:
        pred_file = os.path.join(dataset.model_path, "gic_pred.json")
...
def gen_surround_cam(center, r, num_points, FoVx, FoVy, image, fid):
    ...
    for i in range(num_points):
        ...
        if np.isnan(R).any() or np.isnan(T).any():
            continue
        cam = Camera(
            -1, R, T, FoVx, FoVy, image, image.cpu().numpy(), None, -1, fid=fid
        )
        views.append(cam)
    return views
...
def render_new(
    dataset: ModelParams, pipeline: PipelineParams, phys_args, scene, simulator, bg
):
    ...
    views = scene.getTrainCameras(scale=1.0, cam_idxs=dataset.cam_idxs)
    surround_view = getattr(phys_args, "surround_view", False)

    cam_index = phys_args.view_id
    cams = scene.getTrainCameras(scale=1.0, max_fid_idx=0, cam_idxs=[cam_index])
    cam = cams[0] if cams else None
    if cam is None:
        raise ValueError(f"No camera found with uid {cam_index} in the first frame")
    ...
    if not surround_view:
        ...
        render_views = gen_surround_cam(
            centroid.tolist(),
            r,
            cam_count,
            views[cam_index].FoVx,
            views[cam_index].FoVy,
            views[cam_index].original_image,
            views[cam_index].fid.item(),
        )
        bg = gen_bg(scene)
    ...
    with torch.no_grad():
        ...
        for f in tqdm(range(max_f), desc="Rendering"):
            if surround_view:
                view = render_views[
                    f if f < len(render_views) else len(render_views) - 1
                ]
            else:
                view = render_views[0]
            xyz = simulator.forward(f)
            ...
            results = render(
                view, gaussians, pipeline, background, d_xyz, 0.0, 0.0, False
            )
            ...
            torchvision.utils.save_image(
                blend_img,
                os.path.join(
                    dataset.model_path, "render", "{0:05d}".format(f) + ".png"
                ),
            )
...
if __name__ == "__main__":
    ...
    d_xyz_list = (
        gen_xyz_list(simulator, gs_args.train_frames)
        if gs_args.train_frames > 1
        else None
    )
    ...
    render_new(
        ...
        scene,
        simulator,
        read_bg(phys_args, scene, dataset),
    )
