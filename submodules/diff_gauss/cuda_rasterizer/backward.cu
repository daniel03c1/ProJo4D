...
preprocessCUDA(int P, int D, int M, const float3 *means, const int *radii,
               ...
               const float *view, const float *proj, const glm::vec3 *campos,
               const float3 *dL_dmean2D, glm::vec3 *dL_dmeans, float *dL_dcolor,
               float *dL_ddepth, float *dL_dcov3D, float *dL_dsh,
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderCUDA(
    ...
    const float *__restrict__ depths, const float *__restrict__ accum_alphas,
    const uint32_t *__restrict__ n_contrib,
    const float *__restrict__ dL_dpixels,
    const float *__restrict__ dL_dpixel_depths,
    const float *__restrict__ dL_dpixel_alphas, float3 *__restrict__ dL_dmean2D,
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
      ...
      atomicAdd(&(dL_ddepths[global_id]),
                dpixel_depth_ddepth * dL_dpixel_depth);
...
void BACKWARD::preprocess(
  ...
  preprocessCUDA<NUM_CHANNELS><<<(P + 255) / 256, 256>>>(
      ...
      (glm::vec4 *)rotations, scale_modifier, viewmatrix, projmatrix, campos,
      (float3 *)dL_dmean2D, (glm::vec3 *)dL_dmean3D, dL_dcolor, dL_ddepth,
      dL_dcov3D, dL_dsh, dL_dscale, dL_drot);
}
...
void BACKWARD::render(const dim3 grid, const dim3 block, const uint2 *ranges,
                      ...
                      const float *depths, const float *accum_alphas,
                      const uint32_t *n_contrib, const float *dL_dpixels,
                      const float *dL_dpixel_depths,
                      const float *dL_dpixel_alphas, float3 *dL_dmean2D,
  ...
  renderCUDA<NUM_CHANNELS><<<grid, block>>>(
      ...
      depths, accum_alphas, n_contrib, dL_dpixels, dL_dpixel_depths,
      dL_dpixel_alphas, dL_dmean2D, dL_dconic2D, dL_dopacity, dL_dcolors,
      dL_ddepths);
}