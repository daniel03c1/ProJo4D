...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
    renderCUDA(const uint2 *__restrict__ ranges,
               ...
               float *__restrict__ out_alpha, uint32_t *__restrict__ n_contrib,
...
void FORWARD::render(const dim3 grid, dim3 block, const uint2 *ranges,
                     ...
                     float *out_alpha, uint32_t *n_contrib,
                     const float *bg_color, float *out_color,
                     float *out_depth) {
  renderCUDA<NUM_CHANNELS><<<grid, block>>>(
      ranges, point_list, W, H, means2D, colors, depths, conic_opacity,
      out_alpha, n_contrib, bg_color, out_color, out_depth);
}
