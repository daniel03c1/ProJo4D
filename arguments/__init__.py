...
def get_combined_args(parser: ArgumentParser):
    ...
    try:
        with open(args_cmdline.config_path, "r") as f:
            ...
            data_dict = config_data.get("data", {})
            phys_args.xyz_min = data_dict.get("xyz_min", None)
            phys_args.xyz_max = data_dict.get("xyz_max", None)
    ...
    # Priority: JSON config > CLI > saved config, EXCEPT for the three schedule
    # args where CLI > JSON config (so users can override without editing JSON).
    _SCHEDULE_CLI_SET = {
        k for k in ("stage_codes", "n_chunk_steps", "n_repeat")
        if vars(args_cmdline).get(k) is not None
    }
    ...
    if gs_dict is not None:
        for k, v in gs_dict.items():
            if v is not None and k not in _SCHEDULE_CLI_SET:
                gs_merged_dict[k] = v

    # Defaults for schedule args not supplied by CLI or config
    for k, default in (("n_chunk_steps", 100), ("n_repeat", 1)):
        if gs_merged_dict.get(k) is None:
            gs_merged_dict[k] = default
