    ...
    def __init__(self, t_multires=6, timenet=True, time_out=30):
        ...
        if timenet:
            self.timenet = nn.Sequential(
                nn.Linear(self.time_input_ch, 256),
...
class MotionBasis(nn.Module):
    def __init__(
        self, time_input_ch, num_basis=10, D=8, W=256, num_attributes=10, init_zero=True
    ):
        ...
        self.linear_t = nn.ModuleList(
            ...
            + [
                nn.Linear(W + time_input_ch * (i in self.skips), W)
                for i in range(D - 1)
            ]
        )

        self.bases = nn.Linear(W, self.num_basis * self.num_attributes)
        if init_zero:
            with torch.no_grad():
                self.bases.weight.zero_()
                self.bases.bias.zero_()
    ...
    def forward(self, t_emb):
        ...
        bases = self.bases(h)
        return bases.reshape(-1, self.num_attributes, self.num_basis)
...
class CoefficientNet(nn.Module):
    def __init__(
        ...
        self.num_basis = num_basis
        self.num_attributes = num_attributes  # 3 + 4 + 3
        ...
        self.linear_x = nn.ModuleList(
            ...
            + [
                nn.Linear(
                    W,
                    self.num_basis * self.num_coeff_set_per_basis * self.num_attributes,
                )
            ]
        )
...
class DeformNetwork(nn.Module):
    def __init__(
        ...
        self.cn = CoefficientNet(
            ...
            D=depth_cn,
            softmax=softmax,
            num_attributes=self.total_n_attrs,
        )
        ...
        self.mb = MotionBasis(
            ...
            D=depth_mb,
            num_attributes=self.total_n_attrs,
            init_zero=init_zero_mb,
        )
        self.dxyz_scale = dxyz_scale
        self.feat_agg_fn = feat_agg_fn  # feature aggregating function
    ...
    def forward(self, x, t):
        ...
        attributes = torch.mean(c_ * basis, dim=2)
        attributes = self.feat_agg_fn(attributes, dim=-1)
        attributes = attributes.squeeze(0)

        outputs = torch.split(attributes, self.num_attributes, dim=-1)
        return [o if o.shape[-1] != 0 else 0.0 for o in outputs]
