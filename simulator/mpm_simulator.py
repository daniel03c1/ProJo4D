...
class MPMSimulator:
    ...
    elasticity = 10
    viscous_fluid = 11  # newtonian
    von_mises = 12  # plasticine, non newtownian
    drucker_prager = 13  # for sands
    neo_hookean = 14  # for kaolin Neo-Hookean
    ...
    def __init__(
        ...
        **kargs,
        ...
        self.stop_grad_cache = ti.field(dtype=self.dtype, needs_grad=False)
    ...
    def set_dx(self, dx):
        ...
        self.max_vol[None] = self.grid_vol[None] * 0.5
    ...
    def project_F(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            if ti.static(self.material == self.viscous_fluid):
                # Scalar-J path -- no SVD; uses its own J floor.
                self.F[p, s + 1][0, 0] = ti.max(self.F_tmp[p][0, 0], 0.05)
            else:
                # All SVD-based materials share one clamped sigma.
                sig = self.clamp_sig_diag(self.sig[p])

                if ti.static(
                    self.material == self.elasticity
                    or self.material == self.neo_hookean
                ):
                    ...
                    if self.non_newtonian == 0:
                        ...
                        y = epsilon_hat_norm - self.yield_stress[None] / (
                            2.0 * self.mu[p]
                        )
                        if y > 0:
                    ...
                    # Restored from upstream gic_clean: cohesion shifts the trace
                    # and sets the tension-cone projection target (no behavior
                    # change at cohesion=0, which is the current default).
                    epsilon = ti.log(sig)
                    trace_epsilon = epsilon.sum()
                    shifted_trace = trace_epsilon - self.cohesion[None] * self.dim
                    if shifted_trace >= 0:
                        epsilon = (
                            ti.Vector.one(self.dtype, self.dim) * self.cohesion[None]
                        )
                    else:
                        epsilon_hat = epsilon - (epsilon.sum() / self.dim)
                        epsilon_hat_norm = self.norm(epsilon_hat)
                        ...
                        epsilon -= (
                            ti.max(delta_gamma, 0) / epsilon_hat_norm
                        ) * epsilon_hat
                    ...
                    raise ValueError(f"Invalid material: {self.material}")
    ...
    def backward_svd(self, gu, gsigma, gv, u, sig, v):
        ...
        for i, j in ti.static(ti.ndrange(self.dim, self.dim)):
            if i != j:
                d = s[j] - s[i]
                F[i, j] = d / (d * d + 1e-2)  # to stabilize 1/d
    ...
    @ti.func
    def clamp_sig_diag(self, sig_mat):
        # sig_mat is the diagonal SVD output (self.sig[p]); return a clamped diag vector.
        return ti.Vector(
            [
                ti.max(sig_mat[0, 0], 0.05),
    ...
    def p2g(self, s: ti.i32):
        ...
        for p in range(self.n_particles[None]):
            ...
            if ti.static(self.material == self.elasticity):
                # Restored from upstream gic_clean: strain-rate damping term in
                # the elastic Kirchhoff stress (no behavior change at damping=0).
                J = new_F.determinant()
            ...
            elif ti.static(self.material == self.neo_hookean):
                J = new_F.determinant()
                scale = (self.lam[p] * (J - 1) - self.mu[p]) * J
                stress = self.mu[p] * (
                    new_F @ new_F.transpose()
                ) + scale * ti.Matrix.identity(self.dtype, self.dim)
            elif ti.static(self.material == self.viscous_fluid):
                J = new_F[0, 0]
                kappa = 0.6666666666 * self.mu[p] + self.lam[p]
                stress = (
                    kappa * ti.Matrix.identity(self.dtype, self.dim) * (J - 1 / (J**6))
                )
                grad_v = self.C[p, s]
                epsilon = 0.5 * (grad_v + grad_v.transpose())
            ...
            stress = (
                -self.dt[None] * self.vol[p] * 4 * self.inv_dx[None] ** 2
            ) * stress
    ...
    def grid_op(self, s: ti.i32):
        for I in ti.grouped(self.grid_m):
            if self.grid_m[I] > 1e-1 * self.grid_vol[None]:
                v_out = (
            ...
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
                    for k in ti.static(range(3)):
                        ...
                        new_v += weight * g_v
                        new_C += (
                            4 * weight * g_v.outer_product(dpos) * self.inv_dx[None]
                        )
    ...
    def check_cfl(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            ...
            if self.norm(self.v[p, s]) * self.dt[None] > self.dx[None] * 0.4:
                self.cfl_satisfy[None] = 0
            ...
            if axis_component >= 0 and axis_component <= length and dist <= radius:
                if ti.static(surface == self.surface_sticky):
                    ...
                    if ti.static(surface == self.surface_slip):
                        ...
                        v = v - n * ti.min(normal_component, 0)
    ...
    def set_colliders(self, phys_args):
        for collider_type, collider in phys_args.bc.items():
            if "ground" in collider_type:
                ...
                self.add_surface_collider(point, normal, bc_style)
            elif "cylinder" in collider_type:
                start, end, radius, bc_style = collider
                self.add_cylinder_collider(start, end, radius, bc_style)
