...
def train(
    ...
    scene = Scene(
        dataset,
        gaussians,
        resolution_scales=[dataset.res_scale],
        pcd=pcd,
        load_fix_pcd=True,
        cam_info=cam_info,
    )
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if not viewpoint_stack:
            if iteration < opt.warm_up or d_xyz_list is None:
                # During warm-up, only use cameras from the first frame
                views = scene.getTrainCameras(
                    scale=dataset.res_scale, max_fid_idx=0, cam_idxs=dataset.cam_idxs
                )
            else:
                # Use all available frames
                max_frames = len(d_xyz_list) if d_xyz_list else None
                if max_frames:
                    views = scene.getTrainCameras(
                        scale=dataset.res_scale,
                        max_fid_idx=max_frames - 1,
        ...
        if iteration >= opt.warm_up:
            if opt.reg_scale:
                ...
                diff_scales = torch.nn.functional.relu(
                    scales - grid_size / 16
                ) + torch.nn.functional.relu(grid_size / 32 - scales)
        ...
        with torch.no_grad():
            ...
            if iteration in saving_iterations:
                ...
                shutil.copyfile(
                    ...
                    os.path.join(dataset.model_path, "gic_gaussian.ply"),
                )
...
def psnr_report(
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = [
            ...
            },
            {
                ...
                "cameras": scene.getTrainCameras(
                    scale=res_scale, cam_idxs=scene.cam_idxs
                ),
            },
        ]
        ...
        for config in validation_configs:
            if d_xyz_list is None:
                # Use only cameras from the first frame for validation
                cams = (
                    scene.getTrainCameras(
                        scale=res_scale, max_fid_idx=0, cam_idxs=scene.cam_idxs
                    )
...
def train_gs_with_fixed_pcd(
    ...
    if (
        not check_gs_model(dataset.model_path, saving_iterations, fix_pcd=True)
    ) or force_train:
        train(
            dataset,
            opt,
...
def assign_gs_to_pcd(
    xyz, xyz_opacity, dataset, opt, pipe, cam_info, grid_size=0.12, scene=None
):
    ...
    xyz_opacity = xyz_opacity.reshape(-1, 1).to(scene.gaussians._xyz.device)
